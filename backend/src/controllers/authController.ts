...
export async function doLogin(req: Request, res: Response, next: NextFunction) {
  ...
  if (signer.toUpperCase() === data.wallet.toUpperCase()) {
    ...
    if (!resident) return res.status(StatusCodes.UNAUTHORIZED).send(`Resident not found.`);
  ...
  return res.status(StatusCodes.UNAUTHORIZED).send(`Wallet and secret doesn't match.`);
}
